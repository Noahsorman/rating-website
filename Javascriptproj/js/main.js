...
function calcwinper(){
    ...
    var skip = false;
    for(i = 1; i < 5; i++){
        if(document.getElementById("user_player" + i).value == ""){
            skip = true;
            break;
        }
...
/*Retrieves an array that shows the match scores. 
(ex [1,0,0] for a best of three where team 1 won the first game but lost the other two)*/
function matchscore(){
    var score = [];
    var sum = 0;
...
function findPlayer(n){
    ...
    if (n % 2 == 0 && document.getElementById("user_player" + (n-1)).value != ""){
...
//Retrieves the player's rating to the website table column "ratin".
function findtable_player(){
    for (var i = 1; i < (26 + 1); i++){
        var tablerow = document.getElementById("tPlayer_" + i);
        var tablecells = tablerow.getElementsByTagName("td");
        document.getElementById("tPlayer_" + i + "_rating").innerHTML= playerlist[(tablecells[0].innerText).toLowerCase()].rating;    
    }
}
...
function hocusPocus(){
    ...
    document.getElementById("showconfirmed").innerHTML = "";
}
